...
> ️ **上位机侧的发送忙语义必须显式定下**（2026-09-16 整支评审提出，写上位机计划前须落实）：第 3 行要求上位机在点选套时**连发两帧**（`0x0307` 后紧接 `0x0306`），这与设备侧 C1 是**同一形状的风险**——若 libusb 发送路径同样「忙即拒发」，第二帧会被丢掉，而丢掉的恰好是读回请求，界面就永远刷不出设备值。
>
> ⚠️ **设备侧同样不容忍背靠背帧**（2026-09-16 复审补充）：设备 `ls_app_poll` 要求累积长度**恰好等于**帧头声明长度才消费，而 USB OUT 是逐次传输累加的。若上位机连发的两帧落进同一趟 poll 窗口，长度不等 → **两帧都不被消费**，接收通路会一直卡到缓冲溢出重置。实际概率低（主循环微秒级 vs USB 全速传输毫秒级），但不能据此假定设备能容忍背靠背帧——`0x0307` 与 `0x0306` 之间应留出间隔，或等 `0x0301` 应答后再发。
>
> 两处都要在写上位机计划时落实，并补一条**上位机→设备方向**的上板判据（§7.2 判据 9 只覆盖反方向）。
...
| O8 | 设备成帧要求累积长度**恰好等于**帧头声明长度才消费 | 未改（既有行为，2026-09-02 重构以来） | 背靠背到达的两帧会一起卡住、直到缓冲溢出重置（§5.4 已据此警示上位机）。加固方向：判据改为「不短于」并处理残帧。**非本特性引入，另立任务** |